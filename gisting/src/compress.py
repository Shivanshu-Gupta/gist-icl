...
get_is_t5 = lambda model_name_or_path: "llama" in model_name_or_path.lower()
get_is_llama = lambda model_name_or_path: "t5" in model_name_or_path.lower()
...
def compress_using_model(
    instruction, gist_token, num_gist_tokens,
...
def compress(
    ...
    # Load model
    model = get_model(model_name_or_path, cache_dir, precision, base_llama_path)

    # Load tokenizer. It must already have gist token defined.
    tokenizer, gist_token = get_tokenizer(model_name_or_path, model)

    # Compress instruction
    gist_str, instruction_input_ids, gist_activations = compress_using_model(
        instruction, gist_token, num_gist_tokens,
        model, tokenizer, is_t5, is_llama
    )
...
class Compressor:
    ...
    def get_model(self, precision, base_llama_path):
        return get_model(self.model_name_or_path, self.cache_dir, precision, base_llama_path)

    def get_tokenizer(self):
        return get_tokenizer(self.model_name_or_path, self.model)

    def compress(self, instruction):
        gist_str, instruction_input_ids, gist_activations = compress_using_model(
            instruction, self.gist_token, self.num_gist_tokens,
            self.model, self.tokenizer, self.is_t5, self.is_llama
        )
        return gist_activations
