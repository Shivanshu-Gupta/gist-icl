...
def compute_metrics(
    ...
    if args.data.dataset_name != D.ALPACA:
        # em_results = evaluate.load('accuracy').compute(predictions=decoded_preds, references=decoded_labels)
        results['accuracy'] = np.array([pred == label for pred, label in zip(decoded_preds, decoded_labels)]).mean()
